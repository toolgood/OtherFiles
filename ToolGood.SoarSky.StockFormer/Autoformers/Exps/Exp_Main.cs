...
namespace ToolGood.SoarSky.StockFormer.Autoformers.Exps
{
    public class Exp_Main
    {
        ...
        public virtual Autoformer _build_model()
        {
            var model = new Autoformer(args);
            return model;
        }
            ...
            using (var _no_grad = torch.no_grad()) {
                foreach (var dict in vali_loader) {
                    using (var d = torch.NewDisposeScope()) {
                        ...
                        total_loss.append(loss.cpu().item<float>());
                    }
                }
            }
            this.model.train();
        ...
        public virtual Autoformer train(string setting)
        {
            ...
            foreach (var epoch in Enumerable.Range(0, this.args.train_epochs)) {
                ...
                if (early_stopping.early_stop) { Console.WriteLine("Early stopping"); break; }
        ...
        public virtual void test(string setting, bool test)
        {
            ...
            if (test) {
                ...
                var best_model_path = os.path.join(this.args.checkpoints, setting) + "/" + "checkpoint.pth";
                this.model.load(best_model_path);
            }

            var folder_path = "./test_results/" + setting + "/";
            if (!os.path.exists(folder_path)) { os.makedirs(folder_path); }
            ...
            using (var _no_grad = torch.no_grad()) {
                ...
                foreach (var dict in test_loader) {
                    using (var d = torch.NewDisposeScope()) {
                        ...
                        var @true = batch_y.detach().cpu();//.numpy();
                        preds.append(pred);
                        trues.append(@true);
                        if (i % 20 == 0) {
