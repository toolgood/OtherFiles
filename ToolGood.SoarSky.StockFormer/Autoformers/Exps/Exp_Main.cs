        ...
        public virtual Autoformer train(string setting)
        {
            ...
            foreach (var epoch in Enumerable.Range(0, this.args.train_epochs)) {
                ...
                foreach (var dict in train_loader) {
                    ...
                    if ((i + 1) % 10 == 0) {
                        Console.WriteLine("\titers: {0}, epoch: {1} | loss: {2:0.7f}".format(i + 1, epoch + 1, loss.cpu().item<float>()));
                        var speed = (DateTime.Now - time_now).TotalSeconds / iter_count;
                        var left_time = speed * ((this.args.train_epochs - epoch) * train_steps - i);
                        Console.WriteLine("\tspeed: {0:0.4f}s/iter; left time: {1:0.4f}s".format(speed, left_time));
                        iter_count = 0;
